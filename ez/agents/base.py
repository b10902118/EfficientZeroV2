...
class Agent:
    ...
    def update_weights(
        ...
        image_channel = (
            self.config.env.obs_shape[0]
            if self.config.env.image_based
            else self.config.env.obs_shape
        )
        ...
        if self.config.env.image_based:
            obs_batch_raw = torch.from_numpy(obs_batch_ori).cuda().float() / 255.0
        else:
            obs_batch_raw = torch.from_numpy(obs_batch_ori).cuda().float()

        obs_batch = obs_batch_raw[
            :, 0 : n_stack * image_channel
        ]  # obs_batch: current observation
        ...
        with autocast():
            for step_i in range(unroll_steps):
                ...
                # consistency loss
                gt_next_states = model.do_representation(
